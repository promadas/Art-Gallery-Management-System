...
if ($_SERVER["REQUEST_METHOD"] == "POST") {  
    ...
    {  
        ...
        if (empty($_POST["c_id"])) 
    {  
        $c_idErr = " Fill the blank"; 
        $flag = True;
    } 
    if(!$flag) 
    {
    $id = testdata($_POST["id"]);
    $t_title = testdata($_POST["t_title"]);
    $p_price = testdata($_POST["p_price"]);
    $c_id = testdata($_POST["c_id"]);
    
    $users = sql1($id,$t_title,$p_price,$c_id);
    if($users){
     $dml = "Value Inserted";
  }
  }
...
?>
 ...
 <form id="form2" name="form2" action="dml2.php" method="POST" novalidate >
  <br>
  <fieldset>
